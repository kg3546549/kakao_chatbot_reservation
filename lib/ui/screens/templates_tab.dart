...
class TemplatesTab extends StatelessWidget {
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: '전체조회 텍스트 설정',
            onPressed: () =>
                _showTotalTemplateDialog(context, context.read<BotProvider>()),
          ),
          IconButton(
            icon:
                const Icon(Icons.add_circle_outline, color: Color(0xFF40916C)),
            onPressed: () =>
                _showAddItemDialog(context, context.read<BotProvider>()),
          ),
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                          ...
                          Row(
                            children: [
                              IconButton(
                                ...
                                onPressed: () =>
                                    _showEditItemDialog(context, bot, item),
                              ),
                              IconButton(
                                icon: const Icon(Icons.delete_outline,
                                    color: Colors.redAccent, size: 20),
                                onPressed: () =>
                                    _showDeleteConfirm(context, bot, item),
                              ),
                            ],
                          ),
                        ],
                      ...
                      Text('최대 정원: ${item.maxCapacity}명',
                          style: const TextStyle(
                              color: Colors.grey, fontSize: 13)),
                      const SizedBox(height: 16),
                      const Text('응답 메시지 프리뷰',
                          style: TextStyle(
                              fontWeight: FontWeight.bold,
                              fontSize: 12,
                              color: Colors.grey)),
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('모든 항목의 현황을 보여주는 문구입니다.',
  ...
  void _showEditItemDialog(BuildContext context, BotProvider bot, Item item) {
    ...
    final capController =
        TextEditingController(text: item.maxCapacity.toString());
            ...
            children: [
              TextField(
                ...
                decoration: const InputDecoration(
                    labelText: '항목명', labelStyle: TextStyle(fontSize: 13)),
              ),
              const SizedBox(height: 12),
              TextField(
                controller: capController,
                decoration: const InputDecoration(
                    labelText: '최대 정원', labelStyle: TextStyle(fontSize: 13)),
                keyboardType: TextInputType.number,
              ),
              const SizedBox(height: 24),
              const Align(
                alignment: Alignment.centerLeft,
                child: Text('응답 텍스트 (공지 템플릿)',
                    style:
            ...
            onPressed: () async {
              try {
                final updated = Item(
                  ...
                  maxCapacity:
                      int.tryParse(capController.text) ?? item.maxCapacity,
          ...
          ElevatedButton(
            onPressed: () async {
              if (nameController.text.isNotEmpty) {
                try {
                  await bot.addItem(nameController.text,
                      int.tryParse(capController.text) ?? 15);
                  if (context.mounted) Navigator.pop(context);
                } catch (error) {
                  if (context.mounted) _showError(context, error);
                }
